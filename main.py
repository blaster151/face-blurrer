...
class FaceBlurrer:
    def __init__(self):
        ...
        self.face_detection_1 = self.mp_face_detection.FaceDetection(
            model_selection=1,  # longer range
    ...
    def detect_faces(self, image_rgb):
        ...
        test_images = [
            ("original", image_rgb),
            ("brightened", cv2.convertScaleAbs(image_rgb, alpha=1.5, beta=30)),  # Brighten
            ("contrast", cv2.convertScaleAbs(image_rgb, alpha=1.3, beta=0)),     # Increase contrast
            ("equalized", self.equalize_histogram(image_rgb.copy()))              # Histogram equalization
        ]
    ...
    def create_adaptive_hair_mask(self, face_region, height, width):
        # Create base mask
    ...
    def detect_hair_region(self, image, face_bbox):
        ...
        if len(significant_rows) > 0:
            hair_height = significant_rows[-1]
    ...
    def blur_faces(self, input_path, output_path):
        ...
        if detections:
            ...
            for detection in detections:
                ...
                # Apply the mask
                face_region[:] = (blurred_face * mask_3d + face_region * (1 - mask_3d)).astype(np.uint8)
